...
## Step 1: Confirm printer + canvas

Before drafting, lock in these four numbers:

1. **Printer family**: FGL46 (Lemur), FGL44, FGL42, FGL26/24/22 (no flash; image limits apply).
...
Default if the user has not specified otherwise: **FGL46 @ 300 dpi, 2"×5.5" → 600 rows × ~1650 cols**.
...
8. **Scrub printable text** for parser-sensitive characters before placing it in the stream: `<`, `>`, `^`, `:`, `~`. A `<<` mid-text puts the parser into command mode and silently corrupts everything that follows. Replace with safe ASCII (`** TEAR HERE **` not `>> TEAR HERE <<`).
